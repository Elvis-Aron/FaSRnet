...
class PoseTrack(VideoDataset):
    ...
    def __init__(self, cfg, phase, **kwargs):
        ...
        if self.phase != TRAIN_PHASE:  # 如果不是训练
    ...
    def _get_spatiotemporal_window(self, data_item):
        ...
        # temp_in = self.transform(data_numpy)
    ...
    def _load_coco_keypoints_annotations(self):
        ...
        """
        gt_db = []
        for index in self.image_set_index:  # 遍历图像coc_id 集合
            im_ann = self.coco.loadImgs(index)[0]  # 获得图像的注释
            width = im_ann['width']
            ...
            objs = self.coco.loadAnns(annIds)  # 获得关键点注释
